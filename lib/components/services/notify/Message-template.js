  ...
  async sendMessage (input) {
    ...
    for (const i of input) {
      ...
      try {
        ...
        notif.notifyId = response.body.id
        notif.statusCode = response.statusCode
      } catch (e) {
        notif.statusCode = e.statusCode
        notif.error = `${e.statusCode} - ${e.error.errors.map(x => x.message).join(',')}`
      }
