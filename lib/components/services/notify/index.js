      ...
      for (const templateName of event.customTemplates) {
        ...
        for (const { id: customTemplateId } of customTemplatesFilt) {
          ...
          if (sendConfig && sendConfig.offsetType) {
            ...
            let datetime = moment()
            if (offsetType === 'test') {
              datetime.add(offsetLength, 'seconds').toDate()
            } else {
              if (offsetType === 'HRS') {
                datetime.add(offsetLength, 'hours').toDate()
              }

              if (offsetType === 'DAYS') {
                datetime.add(offsetLength, 'days').toDate()
              }

              if (!['HRS', 'DAYS'].includes(offsetType)) {
                console.log(`offsetType of: ${offsetType} not recognised. Defaulting to sending now`)
              }
            }

            console.log(`   now:       ${new Date()}`)
            console.log(`   scheduled: ${datetime} (offset: ${offsetLength} ${offsetType})`)
