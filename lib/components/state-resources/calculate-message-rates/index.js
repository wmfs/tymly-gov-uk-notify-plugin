...
module.exports = class CalculateMessageRates {
  ...
  async run (event, context) {
    ...
    const smsMessageRate = Number(this.registry.get(SMS_MESSAGE_RATE_KEY) || 0)

    if (messageType === 'sms') {
      const numPages = countSmsPages(message)
      const totalMessages = numPages * numRecipients
      const totalCost = numPages * numRecipients * smsMessageRate

      return context.sendTaskSuccess({ numPages, totalMessages, smsMessageRate, totalCost })
    }
