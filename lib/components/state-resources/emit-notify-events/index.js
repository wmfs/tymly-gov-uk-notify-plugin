...
module.exports = class EmitNotifyEvents {
  ...
  async run (event, context) {
    ...
    const recipients = resolveInputPaths(event, this.recipients)
    ...
    for (const eventName of this.eventNames) {
      ...
      for (const templateName of event.customTemplates) {
        ...
        for (const customTemplate of customTemplatesFilt) {
          ...
          const _recipients = recipients.length > 0
            ? recipients.map(recipient => { return { recipient } })
            : await this.notify.listCustomTemplateRecipients(customTemplateId)
          ...
          try {
            const _sent = await this.notify.templates[templateName].sendMessage(_recipients, customTemplateId)
            sent.push(_sent)
          } catch (e) {
            console.log('--------')
