  ...
  async sendMail (event, context, input) {
    ...
    for (const emailAddress of emailAddresses) {
      try {
        ...
        if (response.statusCode !== 201) error.push(emailAddress)
        else sentMail.push(response.body)
      } catch (e) {
        context.sendTaskFailure({ cause: e, error: 'SEND_MAIL_FAIL' })
      }
    }

    if (error.length > 0) context.sendTaskFailure({ cause: new Error(`Failed to create mail for following email addresses: ${error.join(',')}`), error: 'CREATE_MAIL_FAIL' })
